// ============================================================
//  台股技術分析儀表板 Web 版 v1.2 (Gemini AI)
...
function renderWatchlist() {
  ...
  c.innerHTML = WATCH_LIST.map((s, i) => `
    <div class="stock-item" data-index="${i}" onclick="loadStock(WATCH_LIST[${i}])">
...
async function loadStock(stock) {
  ...
  try {
    const history = await fetchYahooHistory(stock.code, stock.type, '4mo');
  ...
  main.innerHTML = `
    <div class="stock-header">
      <div class="stock-title">
        <h2>${stock.name || stock.code} <span class="code">${stock.code}</span></h2>
